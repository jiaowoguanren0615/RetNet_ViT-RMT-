...
from util.losses import DistillationLoss, TokenLabelGTCrossEntropy, TokenLabelCrossEntropy, TokenLabelSoftTargetCrossEntropy

from models import RMT_B, RMT_L, RMT_S, RMT_T, RMT_B_token_label, RMT_L_token_label, RMT_S_token_label, RMT_T_token_label
...
def get_args_parser():
    parser = argparse.ArgumentParser('VisRetNet training and evaluation script', add_help=False)
    parser.add_argument('--batch-size', default=16, type=int)
    parser.add_argument('--epochs', default=2, type=int)
    parser.add_argument('--predict', default=True, type=bool, help='plot ROC curve and confusion matrix')

    # Model parameters
    parser.add_argument('--model', default='RMT_T_token_label', type=str, metavar='MODEL',
                        choices=['RMT_B', 'RMT_L', 'RMT_S', 'RMT_T', 'RMT_B_token_label',
                                 'RMT_L_token_label', 'RMT_S_token_label', 'RMT_T_token_label'],
                        help='Name of model to train')
    ###############################################################################################################
    # Token labeling
    parser.add_argument('--token-label', default=True, type=bool, help='RetNet_token_release')
...
def main(args):
    ...
    # 'none'
    if args.token_label:
        criterion = TokenLabelCrossEntropy(dense_weight=args.dense_weight, cls_weight=args.cls_weight, mixup_active=False)
    else:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        'VisRetNet training and evaluation script', parents=[get_args_parser()])
